...
// Associations for nested includes
if (db.User && db.Address && db.Geo && db.Company) {
  db.User.belongsTo(db.Address, { foreignKey: 'addressId' });
  db.User.belongsTo(db.Company, { foreignKey: 'companyId' });
  db.Address.belongsTo(db.Geo, { foreignKey: 'geoId' });
  db.Address.hasOne(db.Geo, { foreignKey: 'id', sourceKey: 'geoId' });
